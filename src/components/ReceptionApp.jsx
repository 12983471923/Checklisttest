...
export default function ReceptionApp({ userProfile, currentUser }) {
  ...
  useEffect(() => {
    ...
    const unsubscribeRequests = subscribeToRequests(setRequests);
  ...
  useRealtimeNotifications(messages, {
    showToast,
    type: 'message',
    filter: (m) => m.fromDepartment === 'housekeeping' && !m.readByReception,
    formatMessage: (m) => `Housekeeping: ${m.text}`,
  });

  useRealtimeNotifications(requests, {
    showToast,
    type: 'request',
    filter: (r) => r.status === 'completed' && !r.readByHsk,
    formatMessage: (r) => `Request completed: ${r.title}`,
  });
    ...
    <div className="checklist-container">
      ...
      <div className="main-layout">
        ...
        <div className="left-sidebar">
          <HotelInfoCards
            editableBreakfast
            onEditBreakfast={() => setShowBreakfastModal(true)}
          />
          ...
          <RequestsPanel
            ...
            rooms={rooms}
            onNewRequest={(title) => showToast(`Request sent: ${title}`, 'request')}
          />
          <RoomManagement mode="reception" />
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
            <Link to="/housekeeping" className="portal-nav-btn">
              Housekeeping
            </Link>
            <MessagingPanel
            ...
            />
            {unreadMessages > 0 && (
              <span className="hsk-badge hsk-badge-inline">{unreadMessages}</span>
            )}
            <ThemeToggle />
            <WeatherWidget />
            {isAdmin && (
          ...
          >
            ...
            <form
              onSubmit={async (e) => {
                ...
                await updateUserInitials(currentUser.uid, trimmedInitials);
